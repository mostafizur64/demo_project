...
const R_Slider = () => {
    ...
    return (
        <div className="relative h-[70vh] mb-56">
            ...
            {/* Thumbnail box */}
            <div className="absolute -bottom-24 left-1/2 transform -translate-x-1/2 flex space-x-2 -mb-12">
                {images.map((image, index) => (
                    <div
                        key={index}
                        className={`relative w-24 h-24 cursor-pointer ${index !== activeIndex ? 'filter brightness-75' : ''}`}
                        onClick={() => setActiveIndex(index)}
                    >
                        <img
                            src={image}
                            alt={`thumbnail-${index}`}
                            className="w-24 h-24 object-cover border-2 border-white rounded-md"
                        />
                        {index === activeIndex && (
